...
func test_repair_phase_advances_turn_after_completion():
	...
	_gm.repair_subphase = 1 # Simulation of _execute_all_repairs completion state.
	...
	# Force execution
	_gm.rpc_submit_repair_allocations(1, _gm.repair_allocations)
	_gm.rpc_submit_repair_allocations(2, _gm.repair_allocations)
	await get_tree().process_frame
