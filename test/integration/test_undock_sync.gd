...
func test_undock_movement_registration():
	...
	game_manager.add_child(_station)
	game_manager.ships.append(_station)
	...
	game_manager.add_child(_scout)
	game_manager.ships.append(_scout)
	...
	game_manager.register_movement_plan("Scout", path, facing, 0, false)
	...
	assert_eq(game_manager.ships[1].docked_host, null, "Scout.docked_host should be null")
